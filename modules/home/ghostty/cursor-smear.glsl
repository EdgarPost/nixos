// Cursor smear/trail shader for Ghostty
...
const float DURATION = 0.15; // total animation time (slightly faster)
const float TRAIL_SIZE = 0.8; // 0.0 = all corners move together. 1.0 = max smear
const float THRESHOLD_MIN_DISTANCE = 1.5; // min distance to show trail
const float BLUR = 1.0; // blur size in pixels
const float TRAIL_THICKNESS = 1.0;
const float TRAIL_THICKNESS_X = 0.9;

const float FADE_ENABLED = 1.0; // 1.0 to enable fade gradient
const float FADE_EXPONENT = 3.0; // exponent for fade gradient
